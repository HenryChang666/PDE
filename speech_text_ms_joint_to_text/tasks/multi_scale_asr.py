...
from ..data.s2t_multi_scale_dataset import (
    S2TMSDataConfig,
    S2TMSDataset,
    S2TMSDatasetCreator,
)
...
@register_task("multi_scale_asr")
class MultiScaleASRTask(LegacyFairseqTask):
    @classmethod
    ...
    def __init__(self, args, tgt_dict, char_dict=None, phone_dict=None):
        super().__init__(args)
        self.tgt_dict = tgt_dict
        self.data_cfg = S2TMSDataConfig(Path(args.data) / args.config_yaml)
    ...
    def setup_task(cls, args, **kwargs):
        data_cfg = S2TMSDataConfig(Path(args.data) / args.config_yaml)
        ...
        char_dict = None
        char_vocab_filename = getattr(data_cfg, "char_vocab_filename", None)
        if char_vocab_filename is not None:
            dict_path = Path(args.data) / char_vocab_filename
            if not dict_path.is_file():
                raise FileNotFoundError(f"Dict not found: {dict_path.as_posix()}")
            char_dict = Dictionary.load(dict_path.as_posix())
            logger.info(
                f"char dictionary size ({char_vocab_filename}): " f"{len(char_dict):,}"
            )

        phone_dict = None
        phone_vocab_filename = getattr(data_cfg, "phone_vocab_filename", None)
        if phone_vocab_filename is not None:
            dict_path = Path(args.data) / phone_vocab_filename
            if not dict_path.is_file():
                raise FileNotFoundError(f"Dict not found: {dict_path.as_posix()}")
            phone_dict = Dictionary.load(dict_path.as_posix())
            logger.info(
                f"char dictionary size ({phone_vocab_filename}): " f"{len(phone_dict):,}"
            )
    ...
    def load_dataset(self, split, epoch=1, combine=False, **kwargs):
        ...
        self.datasets[split] = S2TMSDatasetCreator.from_tsv(
            self.args.data,
            self.data_cfg,
            split,
            self.tgt_dict,
    ...
    def build_model(self, args, from_checkpoint=False):
        ...
        return super(MultiScaleASRTask, self).build_model(args, from_checkpoint)
