...
from ..data.speech_to_text_ms_joint_dataset import (
    S2TMSJointDataConfig,
    SpeechToTextMSJointDatasetCreator,
)
from fairseq.tasks import register_task
from fairseq.tasks.speech_to_text import SpeechToTextTask
from ..tasks.ms_translation import ms_load_langpair_dataset

logger = logging.getLogger(__name__)
LANG_TAG_TEMPLATE = "<lang:{}>"


@register_task("speech_text_ms_joint_to_text")
class SpeechTextMSJointToTextTask(SpeechToTextTask):
    """
    ...
    def add_args(cls, parser):
        ...
        super(SpeechTextMSJointToTextTask, cls).add_args(parser)
        ###
        parser.add_argument(
            "--parallel-text-data",
            default="",
    ...
    def __init__(self, args, src_dict, tgt_dict, src_word_dict, src_phone_dict, src_char_dict, infer_tgt_lang_id=None):
        super().__init__(args, tgt_dict)
        self.src_dict = src_dict
        self.src_word_dict = src_word_dict
        self.src_phone_dict = src_phone_dict
        self.src_char_dict = src_char_dict
        self.data_cfg = S2TMSJointDataConfig(Path(args.data) / args.config_yaml)
    ...
    def setup_task(cls, args, **kwargs):
        ...
        data_cfg = S2TMSJointDataConfig(Path(args.data) / args.config_yaml)
        tgt_dict_path = Path(args.data) / data_cfg.vocab_filename
        src_dict_path = Path(args.data) / data_cfg.src_vocab_filename
        src_word_dict_path = Path(args.data) / data_cfg.src_word_vocab_filename \
            if args.use_word_ctc and args.use_src_word_ctc else None
        src_phone_dict_path = Path(args.data) / data_cfg.src_phone_vocab_filename \
            if args.use_phone_ctc else None
        src_char_dict_path = Path(args.data) / data_cfg.src_char_vocab_filename \
            if args.use_char_ctc else None
        ...
        src_word_dict = Dictionary.load(src_word_dict_path.as_posix()) if src_word_dict_path is not None else None
        src_phone_dict = Dictionary.load(src_phone_dict_path.as_posix()) if src_phone_dict_path is not None else None
        src_char_dict = Dictionary.load(src_char_dict_path.as_posix()) if src_char_dict_path is not None else None

        print("| src dictionary: {} types".format(len(src_dict)))
        print("| tgt dictionary: {} types".format(len(tgt_dict)))
        if src_word_dict is not None:
            print("| src word dictionary: {} types".format(len(src_word_dict)))
        if src_phone_dict is not None:
            print("| src phone dictionary: {} types".format(len(src_phone_dict)))
        if src_char_dict is not None:
            print("| src char dictionary: {} types".format(len(src_char_dict)))
        ...
        return cls(args, src_dict, tgt_dict, src_word_dict, src_phone_dict, src_char_dict, infer_tgt_lang_id=infer_tgt_lang_id)
    ...
    def load_langpair_dataset(
        ...
        for lp in self.args.langpairs.split(","):
            src, tgt = lp.split("-")
            text_dataset = ms_load_langpair_dataset(
    ...
    def build_src_word_tokenizer(self, args):
        logger.info(f"src-word-pre-tokenizer: {self.data_cfg.src_word_pre_tokenizer}")
        return encoders.build_tokenizer(Namespace(**self.data_cfg.src_word_pre_tokenizer))
    ...
    def build_src_word_bpe(self, args):
        logger.info(f"src_word_tokenizer: {self.data_cfg.src_word_bpe_tokenizer}")
        return encoders.build_bpe(Namespace(**self.data_cfg.src_word_bpe_tokenizer))
    ...
    def load_dataset(self, split, epoch=1, combine=False, **kwargs):
        ...
        src_word_pre_tokenizer = self.build_src_word_tokenizer(self.args)
        src_word_bpe_tokenizer = self.build_src_word_bpe(self.args)

        ast_dataset = SpeechToTextMSJointDatasetCreator.from_tsv(
            self.args.data,
            self.data_cfg,
            split,
            self.tgt_dict,
            src_dict=None if self.speech_only else self.src_dict,
            src_word_dict=self.src_word_dict,
            src_phone_dict=self.src_phone_dict,
            src_char_dict=self.src_char_dict,
    ...
    def char_dictionary(self):
        return self.src_char_dict

    @property
    def phone_dictionary(self):
        return self.src_phone_dict

    @property
    def source_word_dictionary(self):
        return self.src_word_dict
    ...
    def get_batch_iterator(
        ...
        if not isinstance(dataset, MultiModalityDataset):
            return super(SpeechTextMSJointToTextTask, self).get_batch_iterator(
                dataset,
                max_tokens,
                max_sentences,
                max_positions,
