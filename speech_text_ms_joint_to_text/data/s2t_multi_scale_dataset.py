...
class S2TMSDataConfig(S2TDataConfig):
    """Wrapper class for data config YAML"""
    @property
    def char_vocab_filename(self):
        """fairseq char vocabulary file under data root"""
        return self.config.get("char_vocab_filename", None)

    @property
    def phone_vocab_filename(self):
        """fairseq char vocabulary file under data root"""
        return self.config.get("phone_vocab_filename", None)

    @property
    def word_vocab_filename(self):
        """fairseq char vocabulary file under data root"""
        return self.config.get("word_vocab_filename", None)
...
@dataclass
class S2TMSDatasetItem(object):
    index: int
    source: torch.Tensor
    target: Optional[torch.Tensor] = None
    speaker_id: Optional[int] = None
    target_char: Optional[torch.Tensor] = None
    target_phone: Optional[torch.Tensor] = None


class S2TMSDataset(FairseqDataset):
    ...
    def __init__(
        ...
        self.char_texts, self.phone_texts, self.word_texts = char_texts, phone_texts, word_texts
    ...
    def __getitem__(self, index: int) -> S2TMSDatasetItem:
        ...
        target_char = None
        if self.char_dict is not None and self.char_texts is not None:
            target_char = self.char_dict.encode_line(
                self.char_texts[index], add_if_not_exist=False, append_eos=self.append_eos
            ).long()

        target_phone = None
        if self.phone_dict is not None and self.phone_texts is not None:
            target_phone = self.phone_dict.encode_line(
                self.phone_texts[index], add_if_not_exist=False, append_eos=self.append_eos
            ).long()

        speaker_id = None
        if self.speaker_to_id is not None:
            speaker_id = self.speaker_to_id[self.speakers[index]]
        return S2TMSDatasetItem(
            index=index, source=source, target=target, speaker_id=speaker_id,
            target_char=target_char, target_phone=target_phone
        )
    ...
    def collater(
        self, samples: List[S2TMSDatasetItem], return_order: bool = False
    ) -> Dict:
        if len(samples) == 0:
            return {}
        ...
        char_text, char_text_lengths = None, None
        char_text_ntokens = None
        if self.char_dict is not None and self.char_texts is not None:
            char_text = fairseq_data_utils.collate_tokens(
                [x.target_char for x in samples],
                self.char_dict.pad(),
                self.char_dict.eos(),
                left_pad=False,
                move_eos_to_beginning=False,
            )
            char_text = char_text.index_select(0, order)
            char_text_lengths = torch.tensor(
                [x.target_char.size(0) for x in samples], dtype=torch.long
            ).index_select(0, order)
            char_text_ntokens = sum(x.target_char.size(0) for x in samples)

        phone_text, phone_text_lengths = None, None
        phone_text_ntokens = None
        if self.phone_dict is not None and self.phone_texts is not None:
            phone_text = fairseq_data_utils.collate_tokens(
                [x.target_phone for x in samples],
                self.phone_dict.pad(),
                self.phone_dict.eos(),
                left_pad=False,
                move_eos_to_beginning=False,
            )
            phone_text = phone_text.index_select(0, order)
            phone_text_lengths = torch.tensor(
                [x.target_phone.size(0) for x in samples], dtype=torch.long
            ).index_select(0, order)
            phone_text_ntokens = sum(x.target_phone.size(0) for x in samples)
        ...
        out = {
            ...
            "target_char": char_text,
            "target_lengths_char": char_text_lengths,
            "ntokens_char": char_text_ntokens,
            "target_phone": phone_text,
            "target_lengths_phone": phone_text_lengths,
            "ntokens_phone": phone_text_ntokens,
            "target_word": target,
            "target_lengths_word": target_lengths,
            "ntokens_word": ntokens
        }
        if return_order:
            out["order"] = order
        return out
...
class S2TMSDatasetCreator(object):
    # mandatory columns
    KEY_ID, KEY_AUDIO, KEY_N_FRAMES = "id", "audio", "n_frames"
    KEY_TGT_TEXT = "src_word_text"
    # optional columns
    KEY_SPEAKER, KEY_SRC_TEXT = "speaker", "src_text"
    KEY_SRC_LANG, KEY_TGT_LANG = "src_lang", "tgt_lang"
    KEY_CHAR_TEXT, KEY_PHONE_TEXT = "src_char_text", "src_text"
    # default values
    DEFAULT_SPEAKER = DEFAULT_SRC_TEXT = DEFAULT_LANG = ""
    DEFAULT_CHAR_TEXT = DEFAULT_PHONE_TEXT = ""
    ...
    def _from_list(
        ...
        char_texts = [s.get(cls.KEY_CHAR_TEXT, cls.DEFAULT_CHAR_TEXT) for s in samples]
        phone_texts = [s.get(cls.KEY_PHONE_TEXT, cls.DEFAULT_PHONE_TEXT) for s in samples]
        return S2TMSDataset(
            split_name,
            is_train_split,
            cfg,
    ...
    @classmethod
    def get_size_ratios(
        cls, datasets: List[S2TMSDataset], alpha: float = 1.0
    ) -> List[float]:
